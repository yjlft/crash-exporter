...
{ 
    ...
    if(m_sAppVersion.IsEmpty())
    {
        ...
        if(m_sAppVersion.IsEmpty())
        {
            ...
            m_sAppVersion = _T("version not specified");
        }
    }
...
{  
	...
	//recode Callstack
	ShowCallstack(GetCurrentThread(), pExceptionInfo->pexcptrs->ContextRecord);

	m_pCrashDesc->m_dwStackWalkerOffs = PackString(GetStackWalkerInstance()->GetStackWalkerInfo());
...
COutStackWalker* CCrashHandler::m_psw=NULL;
...
{
	GetStackWalkerInstance()->OnOutputInit();
