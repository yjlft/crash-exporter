...
{ 
	...
	// We should send report immediately
...
int CCrashInfoReader::UnpackCrashDescription(CErrorReportInfo& eri)
{
	...
	UnpackString(m_pCrashDesc->m_dwStackWalkerOffs, m_sStackWalkerInfo);
	...
	while(dwOffs<m_pCrashDesc->m_dwTotalSize)
	{
		...
		else if(memcmp(pHeader->m_uchMagic, "REG", 3)==0 ||memcmp(pHeader->m_uchMagic, "STR", 3)==0)
		{
			// Skip REG string
		}
		else
...
// Returns last error message.
CString CCrashInfoReader::GetStackWalkerInfo()
{
	return m_sStackWalkerInfo;
}