...
//
CWindowIterator::CWindowIterator(DWORD nAlloc): m_current(0), m_count(0)
{
	assert(nAlloc>0);
...
// bForce=TRUE to force kill.
//
BOOL CFindKillProcess::KillProcess(DWORD pid, BOOL bForce)
{
	...
	if (hp) 
	{
		...
		{
			if (bForce)  // didn't die: force kill it if zap requested
